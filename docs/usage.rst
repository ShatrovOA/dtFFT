...
- Utilize brick data decomposition. (Data is distributed in 2 dimensions for 2D plan and in 3 dimensions for 3D plan.)
...
During plan creation, ``dtFFT`` attempts to further redistribute the local brick across the :math:`P_0` ranks (keeping :math:`n_y` and :math:`n_z` fixed) to enable the required transposition/reshape operations. It tries the following strategies in order:
...
User-controlled redistribution (Cartesian communicators)
...
- **1D communicator**: redistributes :math:`P_0` by splitting the last dimension: :math:`n_z` for 3D bricks and :math:`n_y` for 2D blocks.
- **2D communicator**:
...
This effort option extends ``DTFFT_MEASURE`` by selecting the best-performing communication backend for transpositions between MPI ranks.

DTFFT_EXHAUSTIVE
________________

This maximum-effort option extends ``DTFFT_PATIENT`` by including kernel autotuning (both host and GPU, depending on the build) and selecting the best-performing backend for reshape operations.

.. note:: Kernel autotuning can be enabled with all efforts below ``DTFFT_EXHAUSTIVE`` by setting the ``enable_kernel_autotune`` field of :f:type:`dtfft_config_t` to ``true``.
...
.. list-table:: Configuration parameters
   ...
   * - ``enable_fourier_reshape``
     ...
     - ``.false.``
     -
     - Execute reshapes from pencils to bricks in Fourier space. Useful for operations between transforms but reduces performance.
...
For 3D plans, :f:func:`get_local_sizes` does not detail the intermediate Y-direction layout. More generally, for both 2D and 3D plans, detailed layout information (for example, to integrate an external FFT implementation) can be retrieved via the ``pencil`` interface (see `Retrieving memory layouts (Pencils)`_ below).
...
For detailed layout information in 2D and 3D plans (for example, to integrate an external FFT implementation), use the :f:func:`get_pencil` method. In 3D plans this is also how you can query intermediate layouts such as the Y-pencil distribution.

The requested layout is selected via :f:type:`dtfft_layout_t` (Fortran/C) or ``dtfft::Layout`` (C++).
...
``DTFFT_LAYOUT_X_PENCILS_FOURIER`` is meaningful only for R2C plans: it describes the distribution and local extents of the Fourier-space X-pencil representation (which may differ from real-space X-pencils, e.g., due to reduced extent along the transformed dimension). For non-R2C plans, requesting this layout returns :f:var:`DTFFT_ERROR_INVALID_LAYOUT`.
...
- **starts**: Local start indices (0-based) in natural Fortran order. In C/C++, only the first ``ndims`` elements of ``starts[3]`` are defined.
- **counts**: Local element counts (in elements) in natural Fortran order. In C/C++, only the first ``ndims`` elements of ``counts[3]`` are defined.
...
There are three primary methods to execute a plan in ``dtFFT``: ``execute``, ``transpose`` and ``reshape``. First one performs full cycle of transformations, while others may be useful for custom workflows requiring only data transposition or reshaping, e.g., when using external FFT libraries or performing custom data manipulations.

.. note::

  On a CUDA platform all plan execution functions operate asynchronously. When either function returns, computations are queued in a CUDA stream but may not be complete. Full synchronization with the host requires calling ``cudaDeviceSynchronize``, ``cudaStreamSynchronize``, or ``!$acc wait`` (for OpenACC). During execution, ``dtFFT`` may use multiple CUDA streams, but the final computation stage always occurs in the stream returned by :f:func:`get_stream`. Thus, synchronization may be unnecessary if users submit additional kernels to that stream.

Execute
-------

This method executes a plan by optionally reshaping data from bricks to pencils (and vice versa), transposing it, and optionally performing FFTs based on the specified ``execute_type``. It supports in-place execution; the same pointer can be safely passed to both ``in`` and ``out``. To optimize memory usage, ``dtFFT`` uses the ``in`` buffer as intermediate storage, overwriting its contents. Users needing to preserve original data should copy it elsewhere.


Following ``execute_type`` options are supported:

- ``DTFFT_EXECUTE_FORWARD``: Forward execution, transforming data from real to Fourier space.
- ``DTFFT_EXECUTE_BACKWARD``: Backward execution, transforming data from Fourier to real space.
...
The split-phase API is primarily useful for **host** plans with MPI-based backends (e.g., :f:var:`DTFFT_BACKEND_MPI_DATATYPE`, :f:var:`DTFFT_BACKEND_MPI_P2P`, :f:var:`DTFFT_BACKEND_MPI_A2A`) to overlap communication with computation. Regardless of platform, after :f:func:`transpose_start` returns, both ``in`` and ``out`` buffers must remain valid and must not be modified until :f:func:`transpose_end` completes.
...
- Executes a local transposition/packing kernel. On a single process, this completes the task and control returns to the user.
- Performs data redistribution using the selected backend (e.g., MPI, NCCL). Differences between backends begin at this step.
- Final step executes data unpacking kernel to rearrange received data into the ``out`` buffer.

In the Generic version, the ``in`` buffer may serve as intermediate storage and its contents may be modified (in particular on CUDA plans). If you need to preserve ``in``, copy it before calling :f:func:`transpose`.
...
This method redistributes data between **brick** and **pencil** decompositions, writing the result into the ``out`` buffer.
It can be invoked either synchronously via :f:func:`reshape` or as a split-phase operation via :f:func:`reshape_start` followed by :f:func:`reshape_end` (managed via a ``dtfft_request_t`` handle).

.. note::
  Reshape operations are available only for plans created with **bricks decomposition** (i.e., when the plan is created from a :f:type:`dtfft_pencil_t` describing a brick layout).
  Calling :f:func:`reshape` on a plan without reshape support returns :f:var:`DTFFT_ERROR_RESHAPE_NOT_SUPPORTED`.

Reshape operations may use a **separate backend** that is independent of the main transpose backend.
The backend type is the same (i.e., :f:type:`dtfft_backend_t`), and the set of supported backend values is the same as for transpose.
The reshape backend can be configured via ``reshape_backend`` in :f:type:`dtfft_config_t` or via the environment variable ``DTFFT_RESHAPE_BACKEND`` (see :doc:`environ`).
To inspect the selected backends at runtime, use :f:func:`get_backend` (transpose) and :f:func:`get_reshape_backend` (reshape).
...
Supported ``reshape_type`` values include:
...
.. note::
  As with :f:func:`transpose` (Generic version), reshape may use the ``in`` buffer as temporary storage and its contents may be modified.
  If you need to preserve ``in``, copy it before calling :f:func:`reshape`.

Example
_______

Below is an example of a full custom 3D FFT workflow using :f:func:`reshape` and :f:func:`transpose` together with an external (1D) FFT implementation.
The FFT calls shown are placeholders: use FFTW/MKL/cuFFT/VkFFT (or another library) to execute 1D transforms along the currently aligned dimension.
...
The auxiliary buffer ``aux`` is optional for all plan execution functions. If ``aux`` is not provided (``NULL`` / not present), ``dtFFT`` will allocate internal scratch space whenever the selected operation/backend requires it; otherwise it will be ignored.
...
- :f:func:`get_reshape_backend`: Retrieves the backend used for reshape operations, which may differ from the main transpose backend. This is particularly relevant for plans created with bricks decomposition and configured via :f:type:`dtfft_config_t` (see `Setting Additional Configurations`_).
