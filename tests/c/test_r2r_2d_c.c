...
int main(int argc, char *argv[]) 
{
  ...
  int f_kinds[2] = {FFTW_REDFT10, FFTW_REDFT10};
  int b_kinds[2] = {FFTW_REDFT01, FFTW_REDFT01};
  ...
  if(comm_rank == 0) {
    printf("----------------------------------------\n");
    printf("| DTFFT test C interface: r2r_2d       |\n");
    printf("----------------------------------------\n");
    printf("Nx = %d, Ny = %d\n", nx, ny);
  ...
  // Create plan
  plan = dtfft_create_plan_r2r_2d(MPI_COMM_WORLD, ny, nx, f_kinds, b_kinds, DTFFT_ESTIMATE, DTFFT_EXECUTOR_FFTW3);

  dtfft_get_local_sizes(plan, NULL, in_counts, NULL, out_counts);

  in = (double*) malloc(sizeof(double) * in_counts[0] * in_counts[1]);
  out = (double*) malloc(sizeof(double) * out_counts[0] * out_counts[1]);
  check = (double*) malloc(sizeof(double) * in_counts[0] * in_counts[1]);
  ...
  m_err = -1.0;
  for (i = 0; i < in_counts[0]; i++) {
    for (j = 0; j < in_counts[1]; j++) {
        temp = fabs(check[i * in_counts[1] + j] - in[i * in_counts[1] + j]);
  ...
  if(comm_rank == 0) {
    if(m_err < 1e-10) {
      printf("Test 'r2r_2d_c' PASSED!\n");
    } else {
      printf("Test 'r2r_2d_c' FAILED, error = %f\n", m_err);
      return -1;
    }
    printf("----------------------------------------\n");
  }
