...
int main(int argc, char *argv[]) 
{
  ...
  if(comm_rank == 0) {
    printf("----------------------------------------\n");
    printf("| DTFFT test C interface: r2c_2d       |\n");
    printf("----------------------------------------\n");
    printf("Nx = %d, Ny = %d\n", nx, ny);
  ...
  // Create plan
  plan = dtfft_create_plan_r2c_2d(MPI_COMM_WORLD, ny, nx, DTFFT_ESTIMATE, DTFFT_EXECUTOR_FFTW3);

  // Get local sizes
  int alloc_size = dtfft_get_local_sizes(plan, NULL, in_counts, NULL, out_counts);

  // Allocate buffers
  inout = (double*) malloc(2 * sizeof(double) * alloc_size);
  ...
  dtfft_execute_r2c(plan, inout, inout, NULL);
  tf += MPI_Wtime();

  // Normalize
  for (i = 0; i < 2 * out_counts[0] * out_counts[1]; i++) {
    inout[i] /= (double) (nx * ny);
  }

  // Backward (DTFFT_TRANSPOSE_IN) transpose
  double tb = 0.0 - MPI_Wtime();
  dtfft_execute_c2r(plan, inout, inout, NULL);
  tb += MPI_Wtime();
  ...
  if(comm_rank == 0) {
    if(max_error < 1e-10) {
      printf("Test 'r2c_2d_c' PASSED!\n");
    } else {
      printf("Test 'r2c_2d_c' FAILED, error = %E\n", max_error);
    }
    printf("----------------------------------------\n");
  }
