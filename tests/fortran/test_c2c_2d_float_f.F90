...
implicit none
  ...
  complex(R4P), DEVICE_PTR pointer :: in(:), out(:), check(:,:)
  ...
  in_ptr = plan%mem_alloc(alloc_size * element_size)
  out_ptr = plan%mem_alloc(alloc_size * element_size)
