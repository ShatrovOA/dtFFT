...
implicit none
  ...
  integer(I4P), parameter :: nx = 45, ny = 37, nz = 29
  integer(I4P) :: comm_size, comm_rank, i, j, k
  type(dtfft_plan_r2c_3d) :: plan
  integer(I4P) :: in_counts(3), alloc_size
  ...
  allocate(in(in_counts(1),in_counts(2), in_counts(3)), source = 0._R8P)

  allocate(check, source = in)
  
  allocate(out(alloc_size), source = (0._R8P, 0._R8P))
  ...
  if(comm_rank == 0) then
    if(max_error < 1.e-10) then
      write(output_unit, '(a)') "Test 'r2c_3d' PASSED!"
    else
      write(error_unit, '(a, f16.10)') "Test 'r2c_3d' FAILED... error = ", max_error
    endif
    write(output_unit, '(a)') "----------------------------------------"
  endif
