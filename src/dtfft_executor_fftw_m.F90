...
#ifdef DTFFT_WITH_OPENMP
    logical, save :: CREATED_WITH_THREADS = .false.
#endif
...
#ifdef DTFFT_WITH_OPENMP
        if ( .not. CREATED_WITH_THREADS ) then
            error_code = fftw_init_threads()
            if ( error_code == 0 ) then
                ! It has been called by user
