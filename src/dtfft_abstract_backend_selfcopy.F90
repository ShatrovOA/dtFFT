...
module dtfft_abstract_backend_selfcopy
!! This module defines Abstract GPU Backend with self-copying `abstract_backend_selfcopy` 
!! and its pipelined version `abstract_backend_pipelined`
use cudafor
use iso_fortran_env
...
public :: abstract_backend_selfcopy
public :: abstract_backend_pipelined

  type, abstract, extends(abstract_backend) :: abstract_backend_selfcopy
  ...
  end type abstract_backend_selfcopy

  type, abstract, extends(abstract_backend_selfcopy) :: abstract_backend_pipelined
  ...
  end type abstract_backend_pipelined

  interface
    subroutine createPrivateInterface(self, helper)
    !! Creates private interface for self-copying backend
    import
      class(abstract_backend_selfcopy), intent(inout) :: self       !< Self-copying backend
...
contains
  ...
  !! Creates self-copying backend
    ...
    call self%create_selfcopy(helper)
  ...
  !! Sets unpack kernel for pipelined backend
    class(abstract_backend_pipelined),  intent(inout)   :: self           !< Pipelined backend
...
end module dtfft_abstract_backend_selfcopy