...
contains
  ...
  !! Creates FFT plan via vkFFT Interface
    ...
    case ( FFT_R2R )
      ...
      endselect
    endselect
    call vkfft_create(fft_rank, dims, precision, how_many, r2c, int(0, int8), dct, dst, get_user_stream(), self%plan_forward)
    if ( self%is_inverse_required ) then
      call vkfft_create(fft_rank, dims, precision, how_many, int(0, int8), r2c, dct, dst, get_user_stream(), self%plan_backward)
    endif
