...
{
    if (!plan || !*plan)
        return DTFFT_ERROR_PLAN_NOT_CREATED;
...
dtfft_get_aux_size_reshape(dtfft_plan_t plan, size_t* aux_size)
{
    if (!aux_size)
        return DTFFT_ERROR_INVALID_USAGE;
    return (dtfft_error_t)dtfft_get_aux_size_reshape_c(plan, aux_size);
}
