...
public :: abstract_backend, backend_helper
    ...
    !! The most Abstract Backend
        ...
        logical                         :: is_fused                 !! If backend is fused
        ...
        class(abstract_kernel), pointer :: pack_kernel              !! Kernel for packing data
    ...
    contains
        ...
        procedure,  non_overridable,                pass(self) :: set_pack_kernel  !! Sets pack kernel for fused backend
        procedure,  non_overridable,                pass(self) :: set_unpack_kernel!! Sets unpack kernel for pipelined backend
        procedure,  non_overridable,                pass(self) :: wait
...
contains
    ...
    !! Creates Abstract Backend
        ...
        self%buffer_size = max(sum(self%send_floats), sum(self%recv_floats))

        self%backend = backend
        self%is_fused = is_backend_fused(backend) .or. is_backend_compressed(backend)
        self%is_pipelined = is_backend_pipelined(backend) .or. self%is_fused
        self%is_selfcopy = (self%is_pipelined .or. is_backend_mpi(backend)) .and. backend /= DTFFT_BACKEND_CUFFTMP_PIPELINED .and. backend /= DTFFT_BACKEND_NCCL_COMPRESSED .and. .not.self%is_fused
        ...
        if ( self%is_fused .and. backend /= DTFFT_BACKEND_NCCL_COMPRESSED) self%aux_bytes = self%aux_bytes * 2

        self%self_copy_bytes = 0_int64
        if (self%is_selfcopy .or. backend == DTFFT_BACKEND_NCCL_COMPRESSED .or. self%is_fused) then
    ...
    !! Executes Backend
        ...
        type(c_ptr),                intent(in)      :: aux          !! Aux pointer
        type(async_exec_t),         intent(in)      :: exec_type    !! Type of async execution
        integer(int32),             intent(out)     :: error_code   !! Error code
        integer(int32) :: dummy
        real(real32), pointer, contiguous :: pin(:), pout(:), paux(:)
        integer(int64) :: aux_size

#if defined(DTFFT_DEBUG)
        if ( self%is_fused .and. ( .not.associated(self%pack_kernel) .or. .not.associated(self%unpack_kernel)) ) then
            INTERNAL_ERROR("abstract_backend: .not.associated kernel")
        endif
        if ( self%is_pipelined .and. is_null_ptr(aux) ) then
            INTERNAL_ERROR("abstract_backend: self%is_pipelined .and. is_null_ptr(aux)")
        endif
#endif

        call c_f_pointer(in, pin, [self%buffer_size])
        call c_f_pointer(out, pout, [self%buffer_size])
        if ( is_null_ptr(aux) ) then
            paux => pin
        else
            aux_size = self%buffer_size;    if( self%is_fused ) aux_size = aux_size * 2
            call c_f_pointer(aux, paux, [aux_size])
        endif

        if (.not. self%is_selfcopy) then
            call self%execute_private(pin, pout, stream, paux, error_code)
        ...
        if ( self%self_copy_bytes > 0 .and. self%platform == DTFFT_PLATFORM_CUDA ) then
            if ( self%is_pipelined ) then
                call self%execute_self_copy(pin, paux, stream)
                call self%unpack_kernel%execute(aux, out, self%copy_stream, self%comm_rank + 1)
            else
                call self%execute_self_copy(pin, pout, stream)
            endif
        endif
#endif
    ...
    !! Executes self copy
        ...
        REGION_END("self_copy")
    end subroutine execute_self_copy

    subroutine wait(self, stream)
        class(abstract_backend),    intent(inout)   :: self       !! Abstract backend
        type(dtfft_stream_t),       intent(in)      :: stream     !! CUDA stream
...
#ifdef DTFFT_WITH_CUDA
        if ((self%is_selfcopy .or. self%backend == DTFFT_BACKEND_NCCL_COMPRESSED) .and. self%platform == DTFFT_PLATFORM_CUDA) then
    ...
    subroutine set_pack_kernel(self, pack_kernel)
    !! Sets unpack kernel for pipelined backend
        class(abstract_backend),        intent(inout)   :: self           !! Pipelined backend
        class(abstract_kernel), target, intent(in)      :: pack_kernel    !! Kernel for packing data
        self%pack_kernel => pack_kernel
    end subroutine set_pack_kernel
