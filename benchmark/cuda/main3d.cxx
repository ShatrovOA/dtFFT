...
void print_benchmark_results(std::map<std::string, double> &benchmark_results, const std::vector<int> &dims, const std::vector<int> &grid)
{
    int comm_rank;
    ...
    printf("\n");
    printf("===============================================\n");
    printf(" BENCHMARK RESULTS [%ix%ix%i], grid [%ix%ix%i]\n", dims[0], dims[1], dims[2], grid[0], grid[1], grid[2]);
    printf("===============================================\n");
    printf("%-30s %12s\n", "Benchmark", "Time (ms)");
    printf("-----------------------------------------------\n");
...
void run_all(std::vector<int> &dims, bool weak_scaling)
{
    ...
    std::vector<bool> dtype_opts = {true};
    std::vector<bool> z_slab_opts = {false};

    std::vector<int> grid = {0, 0, 0}; // No Cartesian grid
    MPI_Dims_create(comm_size, 3, grid.data());


    // setup_dtfft_config(false);
    // double dtfft_time = run_dtfft_c2c(dims, dtfft::Precision::DOUBLE, dtfft::Executor::CUFFT, false, dtfft::Effort::EXHAUSTIVE, grid);
    // if (dtfft_time > 0) benchmark_results["dtFFT"] = dtfft_time;
    // release_mpi_handles(MPI_COMM_WORLD);
    ...
    double heffte_time = run_heffte<double>(dims, grid);
    ...
    print_benchmark_results(benchmark_results, dims, grid);
}
...
int main(int argc, char *argv[])
{
    ...
    std::vector<std::vector<int>> base_dims_sets = {
        ...
        // {4, 1024, 1024},     // Narrow Y dimension
